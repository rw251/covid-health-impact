...
daily <- read.delim("C:/Users/mdehsrw9/Development/covid-health-impact/data-extraction/one-off-tasks/all_RCs_bydate.txt", header=FALSE, col.names = c("Date","NumCodes"))
...
DF_weekly <- head(DF_weekly, -2)
weeklyTs <- ts(DF_weekly, frequency = 365.25/7, start = c(2010, 2))
par(mfrow=c(2,1))
plot.ts(weeklyTs, ylim=c(0,1200000))
...
plot.ts(weeklyTs, ylim=c(0,1200000), xlim=c(2018,2021), xaxp=c(2018,2021,3))
title("Number of administration codes each week 2018-present")
abline(v = 2020.23, col = "red", lty = 3)
text(2020.23, 30000, "lockdown", pos = 1, srt = 0)

# sma14<-SMA(weeklyTs,n=7)
# plot.ts(sma14)
# 
# comps<-decompose(weeklyTs)
# plot(comps)
# 
# alltsseasonallyadjusted <- weeklyTs - comps$seasonal
# plot(alltsseasonallyadjusted)
